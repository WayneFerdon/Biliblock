...
async function startContainerObserver() {
  new MutationObserver((mutations, observer) => mutations.forEach(mutation => mutation.addedNodes.forEach(node => {
    const str = gE('.bili-subtitle-x-subtitle-panel-text', node);
    if (str) console.log(str.innerText);
  }))).observe(await until(()=>gE('.bpx-player-subtitle-wrap'),100), { childList: true, subtree:true, attribute: true });
}

async function startAISubtitlesObserver() {
  let container = await until(()=>gE('div[class*="_Subtitles_"]'), 100);
